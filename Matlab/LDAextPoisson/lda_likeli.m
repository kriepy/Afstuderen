function [llh,Lalpha,d1,d2,d3,Lgamma] = lda_likeli( d , alpha , lam , gam  )
...
for i=1:length(d) %for every document
    ...
    for j=1:size(t.mat,1)
        ...
        be=[be;sum(log(myNormPoi(w,lam)))]; %this might be sum() instead of prod(), because you use the loglikelihood
    end
    %be=be+0.00000001;
 
    d2=[d2;sum(sum(t.phi.*(be)))];
    if isnan(sum(sum(d2)))
        fprintf(1,'The likelihood is NAN\n')
    end
