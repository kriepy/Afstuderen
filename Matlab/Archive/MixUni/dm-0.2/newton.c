/*
    ...
    $Id: newton.c,v 1.2 2006/01/13 06:59:49 dmochiha Exp $
...
void
newton_beta (double *alpha, double **gammas, int M, int K, int level)
{
	int i, j, t;
		...
		for (i = 0; i < K; i++)
			if (alpha[i] < 0) {
				if (level >= MAX_RECURSION_LIMIT) {
					...
					free(pg);
					free(palpha);
					return newton_beta(alpha, gammas, M, K, 1 + level);
				}
			}
