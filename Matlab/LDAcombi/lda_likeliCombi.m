function [llh,Lalpha,d1,d2,d3,Lgamma] = lda_likeliCombi( d , alpha , beta , gam  )
...
for i=1:length(d) %for every document
    ...
    for j=1:size(t.mat,1)
        ...
        be(j,:)=sum(log(myNormCombi(w,beta.mu,beta.sigma))); %this might be sum() instead of prod(), because you use the loglikelihood
    end
    %be=be+0.00000001;
