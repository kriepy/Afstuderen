function [alpha,beta,likeli,ppl] = ldaCombi(d,k,beta,emmax,demmax)
...
for j = 1:emmax
  ...
  tellerSigPhi=zeros(1,k); % alleen voor de laatste dimensie 
  for i = 1:n
    [gamma,q] = vbemCombi(d{i},beta,alpha,demmax); %q:=phi E-STEP
    gammas(i,:) = gamma; % foreach document a different gamma
    d{i}.phi=q;

    % mu en sigma berekenen (sigma moet alleen voor de laatste dimensie)
    tempTellerMu = tellerMuPhi;
    tempTellerSig = tellerSigPhi;
    for topic=1:k
        tempTellerMu(:,topic)=sum(repmat(q(:,topic),1,l).*d{i}.mat)';
    end  
    tempTellerSig=sum(q(:,:).*(repmat(d{i}.mat(:,end).^2,1,k)));
  ...
  numeratorPhi2=repmat(numeratorPhi,l,1);
  beta.mu=tellerMuPhi./numeratorPhi2;
  beta.sigma=sqrt(tellerSigPhi./numeratorPhi-((beta.mu(end,:)).^2)+0.00000001); 
  ...
  % converge?
   [ppl,l1,l2,l3,l4,l5] = lda_likeliCombi(d, alpha, beta,gammas);
