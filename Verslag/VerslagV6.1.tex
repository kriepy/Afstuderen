...
In figure \ref{fig:PerplTS} we give the perplexity for different length of time-slices for the 5 Houses. We take a again the mean over 10 runs. We took for every house the same amount of days to train and test the data. We can see that with more time-slices the perplexity becomes lower for the hold-out set. For house $H2$ and $H4$ the perplexity is way better for a small amount of time-slices. The perplexities for these two houses drop much faster if the amount of time-slices increases.
...
- gaussian mixture model is a suitable model and it works. We might want to extend this version so that the covariance between the dimensions is well captured. But we must notice that this version need a lot of training data to train correctly.

In the way we represent the features, counting the amount of sensor activations in a time interval, the most logical distribution to use is the Poisson distribution. It becomes more unlikely if the amount of sensor activities has a high value. That is maybe why the Poisson distribution works also better with a small size of time-slices. Oeh that would be awesome, if we could show that.


- Poisson verdeling is meer logisch because of the way the data is used, with another representation of the data it might not that logical anymore to use. Only the time dimension is not so good modelable with this distribution. It might be a good idea to combine the two models together. So 

-smaller timeslices, means more overfitting.

\subsection{Conclusion}
We give two variations of a novel algorithm how to make use of topic model for sensor data, but be able to model the multipl dimensions together.
